...
CREATE TABLE IF NOT EXISTS gmail_accounts (
 id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, email VARCHAR(255) NOT NULL, access_token TEXT, refresh_token TEXT, token_expiry TIMESTAMP WITH TIME ZONE, created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, CONSTRAINT unique_user_gmail UNIQUE (user_id,email)
);
CREATE INDEX IF NOT EXISTS idx_gmail_accounts_user_id ON gmail_accounts(user_id);

-- One-time account recovery codes. Store only SHA-256 hashes, never raw codes.
...
ON recovery_codes(user_id) WHERE used_at IS NULL AND replaced_at IS NULL;


-- One-time Recovery Codes. Only SHA-256 hashes are stored; plaintext codes are never stored.
CREATE TABLE IF NOT EXISTS recovery_codes (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    code_hash CHAR(64) UNIQUE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_recovery_codes_user_active ON recovery_codes(user_id) WHERE used_at IS NULL AND revoked_at IS NULL;
