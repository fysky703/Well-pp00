...
function MainApp() {
  const [theme, setTheme] = useState<'dark' | 'light'>(() => (localStorage.getItem('kh-theme') as 'dark' | 'light') || 'dark');
  ...
  const handleRename = (id: string) => {
    ...
    if (!current) return;
    const name = window.prompt('Rename authenticator', current.codeName);
    if (name && name.trim()) saveAccounts(accounts.map((a) => a.id === id ? { ...a, codeName: name.trim() } : a));
  };
      ...
      {showProfileMenu && (
        <div onClick={() => setShowProfileMenu(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.62)', backdropFilter: 'blur(10px)', zIndex: 20000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: 12 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 520, background: C.surfaceCard, border: `1px solid ${C.border}`, borderRadius: 26, padding: 18, boxSizing: 'border-box', marginBottom: 'env(safe-area-inset-bottom)' }}>
            ...
            <button style={{ ...S.btnGlassSecondary, marginBottom: 10, textAlign: 'left' }} onClick={() => { setShowProfileMenu(false); setShowChangePin(true); }}>Change Vault Lock</button>
            <button style={{ ...S.btnGlassSecondary, marginBottom: 10, textAlign: 'left' }} onClick={() => { setShowProfileMenu(false); setUnlockPin(''); setUnlockError(''); setIsUnlocked(false); }}>Lock Vault Now</button>
