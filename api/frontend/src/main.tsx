...
function MainApp() {
  ...
  const handleQRImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => { const file=event.target.files?.[0]; if(!file)return; try { setScannerError(''); const reader=new BrowserQRCodeReader(); const url=URL.createObjectURL(file); const image=new Image(); image.src=url; await new Promise<void>((resolve,reject)=>{image.onload=()=>resolve();image.onerror=()=>reject(new Error('Unable to load image'));}); const result=await reader.decodeFromImageElement(image); URL.revokeObjectURL(url); const parsed=parseOTPAuthURI(result.getText()); if(!parsed||!isValidBase32(parsed.secret)) throw new Error('Not a valid 2FA QR code'); setScannedResult(parsed); calculateTOTP(parsed.secret).then(setPreviewCode); } catch(err:any){setScannerError(err.message||'Unable to read QR code from image');} };

  const loadGmailAccounts = async () => { const res=await fetch('/api/gmail/accounts',{credentials:'include'}); if(!res.ok)return; const data=await res.json(); setGmailAccounts(data.accounts||[]); if(!selectedGmailAccount&&data.accounts?.[0])setSelectedGmailAccount(data.accounts[0].id); };
  const loadInbox = async (accountId=selectedGmailAccount) => { if(!accountId)return; setInboxLoading(true);setInboxError(''); try { const res=await fetch(`/api/gmail/inbox?accountId=${encodeURIComponent(accountId)}`,{credentials:'include'}); const data=await res.json(); if(!res.ok)throw new Error(data.error||'Failed to load inbox'); setInboxMessages(data.messages||[]); } catch(err:any){setInboxError(err.message||'Failed to load inbox');} finally{setInboxLoading(false);} };
  ...
  return (
    <div style={S.app}>
      ...
      <main style={{ width: '100%', maxWidth: 540, margin: '0 auto', padding: '16px', boxSizing: 'border-box' }}>
        ...
        {activeTab === 'inbox' && (<div><h3 style={{ margin:'0 0 8px 0',fontSize:18,fontWeight:700 }}>Gmail Inbox</h3><p style={{color:C.textSecondary,fontSize:13}}>Read mail from Gmail accounts you explicitly connect.</p><div style={S.card}><button style={S.btnGradBW} onClick={()=>{window.location.href='/api/gmail/connect';}}>+ Add Gmail Account</button><select value={selectedGmailAccount} onChange={(e)=>{const id=e.target.value;setSelectedGmailAccount(id);loadInbox(id);}} style={{width:'100%',marginTop:12,padding:12,borderRadius:12}}><option value="">Select Gmail account</option>{gmailAccounts.map((a)=><option key={a.id} value={a.id}>{a.email}</option>)}</select><button style={{...S.btnGlassSecondary,marginTop:12}} onClick={()=>loadInbox()}>Refresh Inbox</button></div>{inboxError&&<p style={{color:'#FF6F8D'}}>{inboxError}</p>}{inboxLoading&&<p>Loading emails...</p>}{inboxMessages.map((email)=><article key={email.id} style={S.card}><strong>{email.subject||'(No subject)'}</strong><p style={{fontSize:13,color:C.textSecondary}}>{email.from}</p><p style={{fontSize:12}}>{email.snippet}</p><small style={{color:C.textSecondary}}>{email.date}</small></article>)}</div>)}
      ...
      >
        ...
        <button style={{ background:'transparent',border:'none',color:activeTab==='inbox'?C.accent:C.textSecondary,display:'flex',flexDirection:'column',alignItems:'center',gap:3,cursor:'pointer',fontWeight:600,fontSize:11 }} onClick={()=>{setActiveTab('inbox');loadGmailAccounts();}}><span style={{fontSize:20}}>📥</span>Inbox</button>
        <button
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ ...S.glassCard, maxWidth: 390 }}>
            ...
            {scannerError ? (
              ...
              <div>
                ...
                <p style={{ color: C.textSecondary, fontSize: 13, marginTop: 12 }}>Point camera at standard 2FA QR code or upload a QR image.</p>
                <input ref={fileInputRef} type="file" accept="image/*" style={{ display:'none' }} onChange={handleQRImageUpload} />
                <button style={{ ...S.btnGradBW, marginTop: 12 }} onClick={() => fileInputRef.current?.click()}>🖼️ Upload QR Image</button>
