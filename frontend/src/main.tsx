...
const HeaderIcon = ({kind,size=18}:{kind:string,size?:number}) => {
  const paths:any={
    menu:<><path d="M4 6h16M4 12h16M4 18h16"/></>,
    transfer:<><path d="M5 8h12M14 5l3 3-3 3M19 16H7M10 13l-3 3 3 3"/></>,
    ...
    help:<><circle cx="12" cy="12" r="9"/><path d="M9.5 9a2.5 2.5 0 1 1 4.2 1.8c-.8.7-1.7 1.2-1.7 2.7"/><path d="M12 17h.01"/></>
  };
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{paths[kind]}</svg>
};
...
function MainApp() {
    ...
    return (
      ...
      >
        <input
          ...
          style={{ position: 'absolute', opacity: 0.01, pointerEvents: 'auto', width: 1, height: 1 }}
        />
  ...
  useEffect(() => {
    if (hasVault && !isUnlocked && unlockPin.length === pinLength) {
      handleUnlock();
    }
  }, [unlockPin, pinLength, hasVault, isUnlocked]);

  const makeTransferData = () => {
    const payload = accounts.map(({ id, ...rest }) => rest);
    setTransferData('KH2FA:' + btoa(unescape(encodeURIComponent(JSON.stringify(payload)))));
  };

  const importTransferData = (raw: string) => {
    try {
      const clean = raw.startsWith('KH2FA:') ? raw.slice(6) : raw;
      const parsed = JSON.parse(decodeURIComponent(escape(atob(clean))));
      if (!Array.isArray(parsed)) throw new Error('Invalid');
      const next = parsed.map((a: any) => ({
        id: crypto.randomUUID(),
        codeName: String(a.codeName || a.account || 'Authenticator'),
        key: String(a.key || a.secret || ''),
        keyType: a.keyType === 'counter' ? 'counter' : 'time',
        digits: Number(a.digits || 6),
        period: Number(a.period || 30)
      })).filter((a: any) => isValidBase32(a.key));
      setAccounts((prev) => [...prev, ...next]);
      setShowTransfer(false);
      alert(`Imported ${next.length} authenticator account(s).`);
    } catch {
  ...
  return (
    <div style={S.app}>
      ...
      <header className="premium-header" style={{ color: C.textPrimary }}>
        <div className="header-left-premium">
          <button className="icon-only" onClick={() => setShowMainMenu(!showMainMenu)} title="Main Menu"><HeaderIcon kind="menu" /></button>
          <div className="brand-mark">KH <span>Code 2FA</span></div>
          <button className="header-action" onClick={() => { setShowTransfer(true); setTransferMode('export'); }}><HeaderIcon kind="transfer" /> <span>Transfer Code</span></button>
          <button className="header-action" onClick={() => alert('Add or import an authenticator QR code to generate time-based codes.')}><HeaderIcon kind="cap" /> <span>How it Works</span></button>
          <button className="header-action" onClick={() => setActiveTab('settings')}><HeaderIcon kind="settings" /> <span>Settings</span></button>
          <button className="header-action" onClick={() => alert('Help & Feedback')}><HeaderIcon kind="help" /> <span>Help & Feedback</span></button>
        </div>
        <div className="header-search"><input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Search authenticator accounts" /></div>
        <div className="header-right-premium">
          <button className="theme-icon" onClick={toggleTheme} title="Theme">{isDark ? <IconSun size={18}/> : <IconMoon size={18}/>}</button>
          <button onClick={() => setShowProfileMenu(true)} className="profile-trigger" title="Account">
            <img src={user.avatarUrl || 'https://via.placeholder.com/40'} alt="" />
            <span className="profile-name">{user.name}</span>
          </button>
        </div>
      </header>
      {showMainMenu && <div className="main-menu-pop" style={{ color: C.textPrimary, background: C.surfaceCard, borderColor: C.border }}>
        <button onClick={()=>{setActiveTab('vault');setShowMainMenu(false)}}>Vault</button>
        <button onClick={()=>{setActiveTab('inbox');setShowMainMenu(false)}}>Inbox</button>
        <button onClick={()=>{setActiveTab('devices');setShowMainMenu(false)}}>Devices</button>
      </div>}
      ...
      <main style={{ width: '100%', maxWidth: 540, margin: '0 auto', padding: '16px 16px calc(140px + env(safe-area-inset-bottom))', boxSizing: 'border-box', minHeight: '100vh' }}>
                  ...
                  return (
                    <article key={acc.id} style={S.card}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        <button
                          onClick={() => {
                            const name = prompt('Rename Authenticator', acc.codeName);
                            if (name && name.trim()) setAccounts(prev => prev.map(a => a.id===acc.id ? {...a, codeName:name.trim()} : a));
                          }}
                          style={{ background: 'transparent', border: 'none', color: C.accent, cursor: 'pointer', padding: 4 }}
                          title="Rename"
                        >✎</button>
                        <button
        ...
        {activeTab === 'settings' && (
          <div>
            <h3 style={{ margin: '0 0 16px', fontSize: 18 }}>Settings</h3>
            <div style={S.card}>
              <button style={{ ...S.btnGlassSecondary, marginBottom: 12, textAlign: 'left' }} onClick={() => { setNewPin(''); setConfirmNewPin(''); setShowChangePin(true); }}>
                Change Vault Lock
              </button>
              <button style={{ ...S.btnGlassSecondary, textAlign: 'left' }} onClick={toggleTheme}>
      ...
      {/* Account switcher */}
      {showProfileMenu && (
        <div onClick={() => setShowProfileMenu(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.45)', backdropFilter:'blur(8px)', zIndex:20000, display:'flex', alignItems:'flex-start', justifyContent:'flex-end', padding:18 }}>
          <div onClick={e=>e.stopPropagation()} style={{ width:330, maxWidth:'94vw', marginTop:58, background:C.surfaceCard, border:`1px solid ${C.border}`, borderRadius:22, padding:12, color:C.textPrimary }}>
            <div style={{ padding:12, display:'flex', gap:10, alignItems:'center' }}>
              <img src={user.avatarUrl || 'https://via.placeholder.com/40'} style={{width:42,height:42,borderRadius:'50%'}} />
              <div><b>{user.name}</b><div style={{fontSize:12,color:C.textSecondary}}>{user.email}</div></div>
            </div>
            <div style={{height:1,background:C.border,margin:'4px 0 8px'}} />
            <button className="menu-row" onClick={()=>alert('Select another signed-in account. Account switching is available when multiple accounts are connected.')}><HeaderIcon kind="transfer" size={18}/><span>Switch Account</span></button>
            <button className="menu-row" onClick={()=>window.location.href='/api/gmail/connect'}><IconPlus size={18}/><span>Add Account</span></button>
            <button className="menu-row danger" onClick={handleLogout}>⇥ <span>Logout</span></button>
          </div>
        </div>
      )}
      ...
      {showChangePin && (
        <div onClick={() => setShowChangePin(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.72)', backdropFilter: 'blur(10px)', zIndex: 20001, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ ...S.glassCard, maxWidth: 420 }}>
            ...
            <p style={{ color: C.textSecondary, fontSize: 13, marginBottom: 10 }}>Choose 4 or 6 digits, then enter the new PIN twice.</p>
            <div style={{display:'flex',gap:8,marginBottom:8}}>
              {[4,6].map(n=><button key={n} onClick={()=>{setPinLength(n);setNewPin('');setConfirmNewPin('')}} style={{...S.btnGlassSecondary,width:'auto',padding:'8px 14px',borderColor:pinLength===n?C.accent:C.border}}>{n} digits</button>)}
            </div>
      ...
      <button className="floating-add" onClick={() => setShowAddSheet(true)} title="Add Authenticator"><IconPlus size={28} /></button>

      {showTransfer && (
        <div className="modal-layer" onClick={()=>setShowTransfer(false)}>
          <div className="transfer-modal" onClick={e=>e.stopPropagation()} style={{background:C.surfaceCard,borderColor:C.border,color:C.textPrimary}}>
            <div className="transfer-tabs">
              <button onClick={()=>setTransferMode('export')} className={transferMode==='export'?'selected':''}>Export Code</button>
              <button onClick={()=>setTransferMode('import')} className={transferMode==='import'?'selected':''}>Import Code</button>
            </div>
            {transferMode==='export' ? <>
              <h3>Export Authenticator Codes</h3>
              <p>Generate one transfer payload for your authenticator accounts. Keep it private.</p>
              <button style={S.btnGradBW} onClick={makeTransferData}>Generate Transfer Code</button>
              {transferData && <textarea readOnly value={transferData} style={{...S.input,height:130,marginTop:12}} onClick={e=>(e.currentTarget as HTMLTextAreaElement).select()} />}
            </> : <>
              <h3>Import Transfer Code</h3>
              <p>Paste the transfer code or scan/upload a supported transfer QR image.</p>
              <textarea placeholder="KH2FA:..." value={transferData} onChange={e=>setTransferData(e.target.value)} style={{...S.input,height:130}} />
              <button style={{...S.btnGradBW,marginTop:12}} onClick={()=>importTransferData(transferData)}>Import</button>
              <button style={{...S.btnGlassSecondary,marginTop:10}} onClick={()=>{setShowTransfer(false);setShowScannerModal(true)}}>Scan / Upload QR</button>
            </>}
          </div>
        </div>
      )}
