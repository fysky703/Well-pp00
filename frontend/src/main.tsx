...
function MainApp() {
  ...
  useEffect(() => {
    if (!isUnlocked && unlockPin.length === pinLength && !unlockError) handleUnlock();
  }, [unlockPin, pinLength]);
    ...
    return (
      ...
      >
        <input
          ...
          style={{ position: 'absolute', width: 1, height: 1, opacity: 0.01, pointerEvents: 'auto', left: '50%' }}
        />
  ...
  return (
    <div style={S.app}>
      {/* Premium Header */}
      <header style={{...S.header, gap: 14, position:'sticky', top:0, zIndex:9000, backdropFilter:'blur(22px)', flexWrap:'wrap'}}>
        <div style={{display:'flex',alignItems:'center',gap:10,minWidth:220}}>
          <button title="Main Menu" style={{background:'transparent',border:'none',color:C.textPrimary,cursor:'pointer',fontSize:22}}>☰</button>
          <div style={{fontWeight:900,letterSpacing:.3}}>KH Code 2FA</div>
          <button onClick={()=>setShowAddSheet(true)} style={{...S.btnGlassSecondary,width:'auto',padding:'8px 10px'}}>⇄ Transfer Code</button>
          <button onClick={()=>alert('How it Works: add or scan an Authenticator QR, then use the generated code.')} style={{...S.btnGlassSecondary,width:'auto',padding:'8px 10px'}}>🎓 How it Works</button>
          <button onClick={()=>setActiveTab('settings')} style={{...S.btnGlassSecondary,width:'auto',padding:'8px 10px'}}>⚙ Settings</button>
          <button onClick={()=>alert('Help & Feedback')} style={{...S.btnGlassSecondary,width:'auto',padding:'8px 10px'}}>ⓘ Help</button>
        </div>
        <div style={{flex:'1 1 180px',maxWidth:420}}><input placeholder="Search authenticators" onChange={e=>{const q=e.target.value.toLowerCase(); document.querySelectorAll('[data-auth-name]').forEach((el:any)=>el.style.display=(el.dataset.authName||'').includes(q)?'':'none')}} style={{width:'100%',boxSizing:'border-box',padding:'11px 14px',borderRadius:18,border:`1px solid ${C.border}`,background:C.surface,color:C.textPrimary}} /></div>
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <button onClick={toggleTheme} title="Theme" style={{background:'transparent',border:'none',color:C.textPrimary,cursor:'pointer'}}>{isDark ? <IconSun size={20}/> : <IconMoon size={20}/>}</button>
          <button onClick={()=>setShowProfileMenu(true)} style={{display:'flex',alignItems:'center',gap:8,background:'transparent',border:'none',color:C.textPrimary,cursor:'pointer'}} title="Profile Account">
            <img src={user.avatarUrl || 'https://via.placeholder.com/40'} alt="Avatar" style={{width:40,height:40,borderRadius:'50%',border:`1px solid ${C.border}`}} />
      ...
      <main style={{ width: '100%', maxWidth: 540, margin: '0 auto', padding: '16px 16px calc(140px + env(safe-area-inset-bottom))', boxSizing: 'border-box', minHeight: '100vh' }}>
        ...
        {activeTab === 'settings' && (
          <div>
            ...
            <div style={S.card}>
              <div style={{fontWeight:800,marginBottom:8}}>Vault Security</div>
              <p style={{color:C.textSecondary,fontSize:13}}>Change your Vault PIN and choose 4 or 6 digits.</p>
              <button style={S.btnGradBW} onClick={()=>{setNewPin('');setConfirmNewPin('');setShowChangePin(true)}}>Change Vault Lock</button>
              <hr style={{border:'none',borderTop:`1px solid ${C.border}`,margin:'18px 0'}} />
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>Theme Mode</span><button style={{...S.btnGlassSecondary,width:'auto',padding:'8px 14px'}} onClick={toggleTheme}>{isDark?'Dark':'Light'}</button></div>
            </div>
          </div>
        )}
      </main>
      ...
      {showProfileMenu && (
        <div onClick={() => setShowProfileMenu(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.62)', backdropFilter: 'blur(10px)', zIndex: 20000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: 12 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 520, background: C.surfaceCard, border: `1px solid ${C.border}`, borderRadius: 26, padding: 18, boxSizing: 'border-box', marginBottom: 'env(safe-area-inset-bottom)' }}>
            ...
            <button style={{ ...S.btnGlassSecondary, marginBottom: 10, textAlign: 'left' }} onClick={() => alert('Switch Account: choose another signed-in account when available.')}>Switch Account</button>
            <button style={{ ...S.btnGlassSecondary, marginBottom: 10, textAlign: 'left' }} onClick={() => { window.location.href = '/api/gmail/connect'; }}>Add Account</button>
      ...
      {showChangePin && (
        <div onClick={() => setShowChangePin(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.72)', backdropFilter: 'blur(10px)', zIndex: 20001, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ ...S.glassCard, maxWidth: 420 }}>
            ...
            <p style={{ color: C.textSecondary, fontSize: 13, marginBottom: 10 }}>Choose PIN length, then enter it twice.</p>
            <div style={{display:'flex',gap:10,marginBottom:10}}>{[4,6].map(n=><button key={n} onClick={()=>{setPinLength(n);setNewPin('');setConfirmNewPin('')}} style={{...S.btnGlassSecondary,width:'auto',padding:'8px 16px',borderColor:pinLength===n?C.accent:C.border}}>{n} digits</button>)}</div>
      ...
      <button onClick={()=>setShowAddSheet(true)} title="Add" style={{position:'fixed',right:24,bottom:92,width:58,height:58,borderRadius:'50%',border:'none',background:C.accent,color:'#00131a',fontSize:30,fontWeight:800,zIndex:10002,boxShadow:'0 10px 28px rgba(0,140,255,.45)',cursor:'pointer'}}>+</button>
