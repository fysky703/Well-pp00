...
function MainApp() {
  ...
  const [confirmNewVaultPin, setConfirmNewVaultPin] = useState<string>('');
  ...
  const handleUnlock = async (pinToUse = unlockPin) => {
  ...
  // Automatically unlock as soon as all PIN digits are entered.
  useEffect(() => {
    if (!hasVault || isUnlocked) return;
    if (unlockPin.length === pinLength && pinLength > 0) {
      handleUnlock(unlockPin);
    }
  ...
  const handleChangeVaultPin = async () => {
    ...
    if (newVaultPin.length !== pinLength) {
      setChangePinError(`Please enter a ${pinLength}-digit new PIN`);
      return;
    }
    ...
    try {
      const newSalt = 'salt_' + Math.random().toString(36).substring(2);
      const encrypted = await encryptVault(accounts, newVaultPin, newSalt);
      const res = await fetch('/api/vault/change-pin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
          newPinHash: btoa(newVaultPin),
          newPinSalt: newSalt,
      ...
      setCurrentPin(newVaultPin);
      setVaultSalt(newSalt);
      setNewVaultPin('');
      setConfirmNewVaultPin('');
      setChangePinSuccess('Vault lock code changed successfully');
      setTimeout(() => {
        setShowChangePinModal(false);
        setChangePinSuccess('');
      }, 900);
    } catch (error: any) {
      setChangePinError(error.message || 'Failed to change Vault PIN');
    }
  };
    ...
    return (
      <div style={S.center}>
        <div style={S.glassCard}>
          ...
          <PinBoxes value={unlockPin} length={pinLength} onChange={(value) => { setUnlockError(''); setUnlockPin(value); }} onEnter={() => handleUnlock()} />

          {unlockError && <p style={{ color: '#FF6F8D', fontSize: 13, marginBottom: 16 }}>{unlockError}</p>}

          <p style={{ color: C.accent, fontSize: 13, margin: '8px 0 0' }}>Unlocks automatically when all digits are entered</p>
  ...
  return (
    <div style={S.app}>
      ...
      {/* Profile Menu */}
      {showProfileMenu && (
        <div onClick={() => setShowProfileMenu(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '82px 16px 16px', zIndex: 10005, boxSizing: 'border-box' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 440, background: C.surfaceCard, border: `1px solid ${C.border}`, borderRadius: 22, padding: 16, boxShadow: '0 18px 60px rgba(0,0,0,0.55)' }}>
            <div style={{ padding: '6px 6px 14px', borderBottom: `1px solid ${C.border}`, marginBottom: 8 }}>
              <div style={{ fontWeight: 800 }}>{user.name}</div>
              <div style={{ color: C.textSecondary, fontSize: 12, marginTop: 3 }}>{user.email}</div>
            </div>
            <button onClick={() => { setShowProfileMenu(false); window.location.href = '/api/auth/google'; }} style={{ ...S.btnGlassSecondary, textAlign: 'left', marginBottom: 8 }}>Add Gmail Account</button>
            <button onClick={() => { setShowProfileMenu(false); setChangePinError(''); setShowChangePinModal(true); }} style={{ ...S.btnGlassSecondary, textAlign: 'left', marginBottom: 8 }}>Change Vault Lock</button>
            <button onClick={() => { setShowProfileMenu(false); setUnlockPin(''); setUnlockError(''); setIsUnlocked(false); }} style={{ ...S.btnGlassSecondary, textAlign: 'left', marginBottom: 8 }}>Lock Vault Now</button>
            <button onClick={handleLogout} style={{ width: '100%', padding: 13, borderRadius: 14, border: '1px solid rgba(255,111,141,0.35)', background: 'rgba(255,111,141,0.08)', color: '#FF6F8D', fontWeight: 700, cursor: 'pointer' }}>Logout</button>
          </div>
        </div>
      )}
      ...
      {showChangePinModal && (
        <div onClick={() => setShowChangePinModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.78)', backdropFilter: 'blur(14px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 10006, boxSizing: 'border-box' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ ...S.glassCard, textAlign: 'center' }}>
            <div style={{ color: C.accent, marginBottom: 10 }}><IconLock size={40} /></div>
            <h3 style={{ margin: '0 0 6px' }}>Change Vault Lock</h3>
            <p style={{ color: C.textSecondary, fontSize: 13, marginBottom: 14 }}>Set a new {pinLength}-digit Vault PIN.</p>
            <div style={{ textAlign: 'left', marginBottom: 6, color: C.textSecondary, fontSize: 12 }}>NEW PIN</div>
            <PinBoxes value={newVaultPin} length={pinLength} onChange={(v) => { setChangePinError(''); setNewVaultPin(v); }} />
            <div style={{ textAlign: 'left', marginBottom: 6, color: C.textSecondary, fontSize: 12 }}>CONFIRM NEW PIN</div>
            <PinBoxes value={confirmNewVaultPin} length={pinLength} onChange={(v) => { setChangePinError(''); setConfirmNewVaultPin(v); }} onEnter={handleChangeVaultPin} />
            {changePinError && <p style={{ color: '#FF6F8D', fontSize: 13, marginBottom: 12 }}>{changePinError}</p>}
            {changePinSuccess && <p style={{ color: C.accent, fontSize: 13, marginBottom: 12 }}>{changePinSuccess}</p>}
            <button style={S.btnGradBW} onClick={handleChangeVaultPin}>Save New Lock</button>
            <button style={{ ...S.btnGlassSecondary, marginTop: 10 }} onClick={() => setShowChangePinModal(false)}>Cancel</button>
          </div>
        </div>
      )}
